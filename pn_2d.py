...
class P2d(object):
   ...
   def __init__(self, U, P3dAuto, Weight1, Weight2=None, name="", fPnoise=lambda l: 0., fTnoise=lambda l: 0., doT=False, L=None, save=False, nProc=1):
      # copy classes
      self.U = U
      self.Pn = P3dAuto
      ...
      if Weight2 is None:
         self.name = str(Weight1)+name
      else:
         self.name = str(Weight1)+str(Weight2)+name
      ...
      if self.Weight2 is None:
         self.aMin = self.Weight1.aMin
         self.aMax = self.Weight1.aMax
      else:
      ...
      if L is None:
         self.L = np.logspace(np.log10(1.), np.log10(5.e4), 101, 10.)
      else:
         self.L = L
...
#      return result
   ...
   def plotP(self):
      ...
      ax = plt.subplot(111)
      #
      ax.loglog(self.L, self.P+self.Pnoise, 'k', lw=4, label=r'$P_\text{total}$')
      ax.loglog(self.L, self.P, 'k--', lw=2, label=r'$P$')
      ...
      ax.loglog(self.L, factor*(self.P+self.Pnoise), 'k', lw=4, label=r'$P_\text{total}$')
      ax.loglog(self.L, factor*self.P, 'k--', lw=2, label=r'$P$')
   ...
   def plotdPdz(self, l=1.e3):
      ...
      def f(a):
         z = 1./a-1.
         chi = self.U.bg.comoving_distance(1./a-1.)
         return self.Pn.fP1hinterp(l/chi, z)
      P3d_1h = np.array(map(f, A))
      
      def f(a):
         z = 1./a-1.
         chi = self.U.bg.comoving_distance(1./a-1.)
         return self.Pn.fP2hinterp(l/chi, z)
